...
public class EquipeController implements IEquipeController {
    ...
    @Override
    public boolean verificaPessoaExisteEmEQP(int idpessoa, int idequipe) {
        List<rel_pessoa_equipe> listarel = new ArrayList<rel_pessoa_equipe>();
        if (verificaEquipeById(idequipe)) {
            if (PessoaController.getInstance().verificaPessoaById(idpessoa)) {
            ...
            javax.swing.JOptionPane.showMessageDialog(null, "Erro, verifique id equipe!");
        }
        return false;
    }

    @Override
    public boolean verificaFuncao(int idfuncao, int idpessoa, int idprojeto) {
        List<rel_pessoa_equipe> listarel = new ArrayList<rel_pessoa_equipe>();
        if (FuncaoController.getInstance().verificaFuncaoById(idfuncao)) {
            if (PessoaController.getInstance().verificaPessoaById(idpessoa)) {
                if (ProjetoController.getInstance().verificaProjetoById(idprojeto)) {
                    listarel = retornaListaDeRelPessoasEquipeByIdequipe(ProjetoController.getInstance().retornaProjetoById(idprojeto).getIdequipe());
                    for (rel_pessoa_equipe object : listarel) {
                        if(object.getIdpessoa() == idpessoa && object.getIdfuncao() == idfuncao){
                            return true;
                        }
                    }
                } else {
                ...
                javax.swing.JOptionPane.showMessageDialog(null, "Erro, verifique id pessoa!");

            }
        } else {
            javax.swing.JOptionPane.showMessageDialog(null, "Erro, verifique id função!");
